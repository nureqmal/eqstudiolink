...
export async function onRequestGet(context) {
  ...
  const [usersRes, customersRes, remindersRes, paidRes, bookingsRes] = await Promise.all([
    ...
    fetch(`${base}/rest/v1/bookings?select=id&status=eq.confirmed`, { headers: { ...headers, Prefer: "count=exact" } }),
  ]);
  ...
  const stats = {
    ...
    total_bookings: countFrom(bookingsRes),
